...
if(!empty($_REQUEST['id'])) {
	...
	$searchqry 	    = mysqli_query($con,"select * from  uploadjobskrdata_t
							 			where jobsekr_id = '".$id."'");
...
if(!empty($_REQUEST['submit'])) {
	...
	$searchcountqry 	    = mysqli_query($con,"select count( * ) from  uploadjobskrdata_t
							 				where jobsekr_id = '".$id."'");
	$count = mysqli_num_rows($searchcountqry)	;									
	if($count > 0){
		$id = $_REQUEST['id_f'];
		$qry = "update uploadjobskrdata_t set
		...
		$insqry = mysqli_query($con,"insert into  uploadjobskrdata_t set
			user_role               = '2',
			user_id                 = '".$_SESSION['_SESS_USERID']."',
			jobsekr_compName  	    = '".mysqli_real_escape_string($con,$cname)."',
