...
if((empty($f_oPass))||(empty($f_nPass))||(empty($f_cPass))){
	...
	if($Error=='false'){
	  ...
	  $count=mysqli_query($con,"SELECT count(*) 
						FROM 
							users_m_t  
						WHERE 
							user_pwd = ENCODE('".mysqli_real_escape_string($con,$f_oPass)."','vsvtech') 
						AND 
		                      	user_id ='".mysqli_real_escape_string($con,$_SESSION['_SESS_USERID'])."'");
		
		if($count==1)
		{
//			$histid = $db->query("insert into `users_h_t` select * from `users_m_t` where csbMgrId = '".$_SESSION['_SESS_USERID']."'");
			$id = mysqli_query($con,"UPDATE users_m_t
			                     SET 
								 user_pwd = (ENCODE('" . mysqli_real_escape_string($con,$f_nPass) . "','vsvtech'))
								 WHERE 
								   		user_id =". mysqli_real_escape_string($con,$_SESSION['_SESS_USERID'])."");	
